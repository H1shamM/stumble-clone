...
const draft = { summary: "Magma rises, pressure wins.", keyPoints: ["a", "b"] };

describe("enrichReader", () => {
  beforeEach(() => _clearEnrichmentCache());

  it("returns an explainer with image, provenance and source url", async () => {
  ...
  it("returns null on an empty summary", async () => {
    const llm: ExplainerLLM = {
      summarize: vi.fn().mockResolvedValue({ summary: "   ", keyPoints: [] }),
    };
