...
describe("firstImage junk filtering", () => {
  it("rejects images with 'logo', 'icon', 'avatar', 'spacer' in URL", () => {
    ...
    expect(
      firstImage('<img src="https://cdn.test/avatar_123.png" />'),
    ).toBeNull();
  ...
  it("rejects images with small dimensions in attributes", () => {
    // 1x1
    expect(
      firstImage('<img src="https://cdn.test/ok.png" width="1" height="1" />'),
    ).toBeNull();
    // Tiny (likely icons)
    expect(
      firstImage(
        '<img src="https://cdn.test/ok.png" width="16" height="16" />',
      ),
    ).toBeNull();
    expect(
      firstImage(
        '<img src="https://cdn.test/ok.png" width="32" height="32" />',
      ),
    ).toBeNull();
  });
  ...
  it("accepts larger images", () => {
    expect(
      firstImage(
        '<img src="https://cdn.test/hero.png" width="800" height="600" />',
      ),
    ).toBe("https://cdn.test/hero.png");
    // No dimensions provided - default to accepting if URL is clean
    expect(firstImage('<img src="https://cdn.test/hero.png" />')).toBe(
...
const mockReader = (content: string): ReaderResult => ({
  ...
  content,
  textContent:
    "Long enough text content to pass enrichment minimum length checks.".repeat(
      10,
    ),
  length: 500,
});
...
describe("ExplainerService image fallback sequence", () => {
  ...
  it("prefers firstImage from content if it's high quality", async () => {
    const html =
      '<html><head><meta property="og:image" content="https://test.com/og.png" /></head><body><img src="https://test.com/hero.png" width="800" /></body></html>';
  ...
  it("falls back to og:image if firstImage is junk", async () => {
    const html =
      '<html><head><meta property="og:image" content="https://test.com/og.png" /></head><body><img src="https://test.com/logo.png" /></body></html>';
    const service = new ExplainerService(mockLLM, {
      fetchHtml: async () => ({ html }),
      extract: () => mockReader(html),
    });
