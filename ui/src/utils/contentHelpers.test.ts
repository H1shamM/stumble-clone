...
describe('contentHelpers', () => {
  it('should return correct favicon URL for known sources', () => {
    expect(getFaviconUrl('Hacker News')).toBe('https://news.ycombinator.com/favicon.ico');
    expect(getFaviconUrl('Wikipedia')).toBe('https://www.wikipedia.org/favicon.ico');
  });

  it('should return fallback favicon for unknown source', () => {
    expect(getFaviconUrl('Unknown')).toBe('https://www.google.com/s2/favicons?domain=example.com');
  });
